...
public class ChangeSetListTest extends AbstractGitTestCase {
    ...
    private void checkRecentChanges(boolean buttonsVisible) throws Exception {
        final HtmlPage changesPage = goTo(getRecentChangesUrl(project), MIME);
        final HtmlObjects changesObj = getHtmlObjects(changesPage);
        changesObj.assertButtonsExist(true);
        Assert.assertTrue(changesObj.hideElement.isDisplayed()
                || changesObj.showElement.isDisplayed());
    }
